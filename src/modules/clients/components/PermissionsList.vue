    ...
    <div class="show d-none d-sm-block ml-5 mr-4">
      ...
      <v-row class="mt-n4">
        <v-col
          cols="12"
          md="4"
          v-for="system_service in clientsStore.system_services_paginator.list"
          :key="system_service.id"
        >
          <v-card
            elevation="2"
            link
            class="border-color full-height-card page-margins"
          ...
          >
            <v-card-text>
              <div class="align-items">
                ...
                <span class="mt-n2">
                  <v-icon
                    v-if="clientHasSystemService(system_service.id)"
                    color="green"
                    size="33"
                  >
              ...
              <div class="mt-6">
                ...
                <li
                  v-if="
                    system_service.functionalities[0].functionalities.length > 3
                  "
                  class="list-style"
                >
                  <span class="icon-style">★</span>
      ...
      <v-card>
        <v-container>
          ...
          <v-card-text class="mt-n2">
            <li
              v-for="(action, i) in selected_system_service.functionalities[0]
                .functionalities"
              :key="i"
              class="list-style-dialog"
            >
              <span class="icon-style">★</span>
              {{ action.name }}
            </li>
...
const syncSystemServices = async () => {
  const selected_system_services_ids = clientsStore.client_system_services.map(
    (system_service: any) => system_service.id
  );
  const { error } = await clientsStore.syncSystemServices(
    clientsStore.selectedItem.id,
    selected_system_services_ids
  );
  if (!error) {
    await swal.fire({
      icon: "success",
...
const clientHasSystemService = (system_service_id: number) => {
  return clientsStore.client_system_services.some(
    (system_service: any) => system_service.id == system_service_id
  );
};
