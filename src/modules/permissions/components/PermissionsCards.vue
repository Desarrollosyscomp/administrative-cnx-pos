...
<template>
  <div
    ...
    v-for="(item, index) in permissionsStore.list"
    :key="index"
  >
...
const goToEdit = (permissions: PermissionsInterface) => {
  emit("onEdit", {
    name: "PermissionsTable.goToEdit",
    data: { permissions },
  });
};
const unableItem = async (item: PermissionsInterface) => {
  emit("onDeactivate", {
    name: "PermissionsTable.deactivate",
    data: { item },
  });
};
</script>
