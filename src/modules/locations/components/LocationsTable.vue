...
<template>
  <div class="show d-none d-sm-block">
    <v-table>
      ...
      <tbody>
        <tr v-for="(item, index) in locationsStore.list" :key="index">
...
const goToEdit = (locations: any) => {
  emit("onEdit", {
    name: "LocationsTable.goToEdit",
    data: { locations },
  });
};
const unableItem = async (item: any) => {
  emit("onDeactivate", {
    name: "LocationsTable.deactivate",
    data: { item },
  });
};
</script>
